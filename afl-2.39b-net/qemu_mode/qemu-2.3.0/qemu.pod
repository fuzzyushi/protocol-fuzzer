...
B<align=on> is specified then we print a messsage to the user
...
=item B<-semihosting-config [enable=on|off,]target=native|gdb|auto>

Enable semihosting and define where the semihosting calls will be addressed,
to QEMU (C<native>) or to GDB (C<gdb>). The default is C<auto>, which means
C<gdb> during debug sessions and C<native> otherwise (ARM, M68K, Xtensa only).
